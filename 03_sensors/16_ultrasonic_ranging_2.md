...
Redo [Ultrasonic Ranging](./15_ultrasonic_ranging.md) using a pre-built `hcsr04` driver module instead of hand-rolling the trigger/echo timing — much shorter code, same result.
...
*Identical to [Ultrasonic Ranging](./15_ultrasonic_ranging.md)*.
...
SR=SR04(13,14)
```
`SR04(13,14)` does everything [Ultrasonic Ranging](./15_ultrasonic_ranging.md)'s `getSonar()` function did manually — sets up the Trig/Echo pins — but hides it behind one constructor call.
...
- Compare this code's readings against [Ultrasonic Ranging](./15_ultrasonic_ranging.md)'s manual version — they should match closely.
