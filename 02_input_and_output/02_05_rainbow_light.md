...
*Same module, board, and circuit as [LED Pixel](./02_04_led_pixel.md).*

---

## Circuit

*Same circuit as [LED Pixel](./02_04_led_pixel.md) — see that page for the schematic and wiring diagrams.*
...
def wheel(pos):
    ...
    if WheelPos<85:
        ...
        blue=(255-WheelPos*3)
```
Same idea as [Gradient Color Light](./02_03_gradient_color_light.md)'s `wheel()`, but using a 0–255 range (since `np[j]` expects 0–255 color bytes, not 0–1023 PWM duty values) and setting `red`/`green`/`blue` globals instead of writing to PWM pins directly.
